...
class ServiceClient:
    ...
    async def get_uploaded_files(self, project_id: str) -> Dict:
        ...
        logger.info(f"ServiceClient: Using URL http://localhost:8002/api/projects/{project_id}/files")
        logger.info(f"ServiceClient: Headers: {headers}")

        try:
            result = await self._make_request("GET", "project", f"/api/projects/{project_id}/files", headers=headers)
