# =====================================================================================
# Nagarro AgentiMigrate Platform - Optimized Image Builder (Clean Version)
# =====================================================================================
...
foreach ($build in $builds) {
    ...
    if ($exists -and -not $Force) {
        Write-Log "SKIP: [$currentBuild/$totalBuilds] Skipping $($build.Name) - image already exists" "Yellow"
    ...
    try {
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Log "SUCCESS: $($build.Name) built successfully in $([math]::Round($duration, 1)) minutes (Size: $size)" "Green"
            $buildResults[$build.Name] = "SUCCESS"
            
            # Push to registry if specified
            if ($Registry) {
                $registryTag = "$Registry/$($build.Tag)"
                Write-Log "PUSH: Pushing to registry: $registryTag" "Yellow"
...
if ($failed -eq 0) {
    ...
    Write-Log "   - Run: .\start-platform-fast.ps1 -SkipBuild" "Yellow"
