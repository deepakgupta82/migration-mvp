"""AWS RDS adapter for cloud relational database."""
...
class RdsAdapter(RelationalDBInterface):
    """
    AWS RDS adapter for cloud PostgreSQL.
    
    Implements the RelationalDBInterface using asyncpg for RDS PostgreSQL connections.
    Similar to PostgreSQLAdapter but optimized for RDS with SSL and IAM authentication.
    """
    
    def __init__(self, config: Dict[str, Any]):
        """
        Initialize RDS adapter.
        ...
        if not self.host:
            raise ValueError("RDS host is required")
        if not self.username:
            raise ValueError("RDS username is required")
        if not self.password and not self.use_iam_auth:
            raise ValueError("RDS password is required when not using IAM auth")
    ...
    async def _get_iam_token(self) -> str:
        ...
        try:
            ...
            rds_client = boto3.client('rds', region_name=self.config.get('region', 'us-east-1'))
            ...
            raise DatabaseError(
                f"Failed to generate IAM token for RDS: {str(e)}",
                database_type="rds",
                context={
                    "host": self.host,
    ...
    async def connect(self) -> None:
        """Establish connection pool to RDS."""
        try:
            password = await self._get_iam_token()
            ...
            self._pool = await asyncpg.create_pool(
                ...
                server_settings={
                    'application_name': 'agentimigrate-platform'
                }
            )
    ...
    async def execute_query(
        ...
        try:
            ...
            # Handle connection issues that might require token refresh
            if "authentication" in str(e).lower() and self.use_iam_auth:
                # Refresh connection pool with new token
            ...
            raise DatabaseError(
                f"RDS query execution failed: {str(e)}",
                database_type="rds",
                query=query,
    ...
    async def execute_command(
        ...
        try:
            ...
            raise DatabaseError(
                f"RDS command execution failed: {str(e)}",
                database_type="rds",
                query=command,
                context={"parameters": parameters}
            )
    ...
    async def execute_transaction(
        ...
        try:
            ...
            raise DatabaseError(
                f"RDS transaction execution failed: {str(e)}",
                database_type="rds",
