...
# Nagarro's Ascent Platform - Technical Overview (2025-08-19)

## Executive Summary

Nagarro's Ascent is an enterprise-grade, agentic cloud migration assessment platform. It automates complex transformation assessments using specialized AI agent crews, multi-modal knowledge synthesis, and professional deliverables. The platform features:

- **AI-Driven Assessment:** Multi-agent crews orchestrated via CrewAI, leveraging RAG, graph, and hybrid search tools.
- **Professional Deliverables:** Executive-ready Markdown, PDF, and DOCX reports generated by agents and stored in MinIO.
- **Real-Time Intelligence:** Interactive dependency graphs, RAG-powered chat, and live agent/crew monitoring via WebSocket and REST logs.
- **Zero-Trust Security:** Full data isolation, JWT authentication, RBAC, and encrypted API keys.
...
│  Command Center      │ AI Orchestrator   │ State/CRUD      │ PDF/DOCX  │
│  Multi-Tab UI        │ CrewAI, RAG, WS   │ PostgreSQL      │ MinIO     │
│  Real-Time Logs      │ ChromaDB, Neo4j   │ JWT Auth        │ Pandoc    │
└─────────────────────────────────────────────────────────────────────┘
```

### Key Services

- **Frontend Command Center:** React + Mantine UI, multi-tab workspace, project CRUD, file upload, document generation, chat, logs, and service health.
- **Backend Orchestrator:** FastAPI, CrewAI agent orchestration, RAGService (ChromaDB), GraphService (Neo4j), WebSocket streaming, REST logs, document generation, and knowledge base queries.
- **Project Service:** FastAPI, SQLAlchemy, PostgreSQL, JWT, project CRUD, file metadata, LLM config management, and new freeform context fields (overview, client summary, RFP, expectations, deliverables, timelines).
- **Reporting Service:** FastAPI, Pandoc, MinIO, Markdown → PDF/DOCX conversion, artifact storage, download endpoints.

### Data Stores

- **PostgreSQL:** Projects, users, configs, files, deliverables, templates.
...
- Projects now support rich freeform fields: overview, client summary, RFP summary/responses, expectations, deliverables summary, timeline notes.
- These fields are indexed into ChromaDB (as a canonical markdown doc) and Neo4j (as node properties) on update or via explicit reindex endpoint.
- CrewAI agents and RAG queries use this context for synthesis, document generation, and chat.

### Document Processing & Generation

- File uploads are stored in MinIO, parsed to Markdown, and indexed into ChromaDB and Neo4j.
- Document generation (Markdown → PDF/DOCX) is orchestrated by CrewAI agents, with outputs saved to MinIO and downloadable via REST endpoints.
- Exported Documents tab in the UI triggers agentic workflows and provides download links for all formats.
...
- Service health and status are monitored via dedicated endpoints and UI panels.
...
## Recent Enhancements (2025-08-19)

- Added project-level freeform context fields, indexed to vectors/graph, surfaced in agent workflows and chat.
- End-to-end document generation: Markdown → PDF/DOCX via Reporting Service, with download endpoints and MinIO storage.
- Enhanced logs: REST + WebSocket, multi-service filtering, correlation ID, project, time, and level.
- Improved agent/crew orchestration: context-aware synthesis, document generation, and chat.
- UI: Exported Documents tab, service health panel, global log pane, and context editing.

## How to Contribute

See DEVELOPER_ONBOARDING.md for step-by-step guides to add new fields, tools, or workflows.
