...
| CARD_PIPELINE_SCHEMA_VERSION | Version namespace for card vector metadata/signature hashing | v2 |
...
2025-09-27 C3 Cards Pipeline Alignment Upgrade DONE
	- Introduced `CARD_PIPELINE_SCHEMA_VERSION` namespace plus deterministic content signatures to drive incremental regeneration decisions and analytics tagging.
	- Reworked `_build_entity_and_triple_cards` to consume chunk metadata, compute alignment-aware dispersion weighting, and emit structured provenance (snippets, densities, chunk ids).
	- Persisted card metadata as JSON (`metadata_json`) within Weaviate, expanded search helpers to merge stored metadata into responses, and added analytics metrics (`alignment_avg`, `provenance_total`, raw chunk counts).
	- Added unit coverage (`tests/test_card_generation.py`) validating provenance weighting and signature stability.
...
1. Cards Pipeline Phase 2 (follow-ups)
	- DONE: alignment-based weighting, provenance persistence, schema versioning + deterministic regeneration signatures (2025-09-27).
	- TODO: wire weighting metadata into LLM summarization heuristics, expose provenance analytics via dashboard, and exercise incremental regeneration across project pipelines.
